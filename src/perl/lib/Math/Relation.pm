...
sub _addItem($@) {
    ...
    foreach my $item (@_) {
        UNIVERSAL::isa($item, "Math::Multiplet")
...
sub relates($@) {
    ...
    return $this->contains(new Math::Multiplet(@_));
}
...
sub _cut($@) {
    ...
    foreach my $multiplet ($this->items()) {
        ...
        if ($i == $arity) {
            push(@cut_items, new Math::Multiplet(@cut_multiplet));
        }
    }
    ...
    return new Math::Relation($arity, @cut_items);
}
...
sub cut($@) {
    ...
    my $cut = $this->_cut(map(Math::Set::itemKey($_), @_));
