package CTXFryer::FSA::Symbol::ASCII;

use strict;
use warnings;

use base qw(CTXFryer::FSA::Symbol);
...
sub first($) {
    ...
    return new CTXFryer::FSA::Symbol::ASCII($all[0]);
}
...
sub last($) {
    ...
    return new CTXFryer::FSA::Symbol::ASCII($all[-1]);
}
...
sub pred($) {
    ...
    return new CTXFryer::FSA::Symbol::ASCII(chr($ord - 1));
}
