...
sub new($@) {
    ...
    my $this = {
        _interval => [ new FSA::Symbol::Interval() ],
    };
...
sub copy($) {
    ...
    return new FSA::Symbol::Set(@interval);
}
...
sub _toInterval(@) {
    ...
    foreach my $obj (@_) {
        ...
        if (UNIVERSAL::isa($obj, "FSA::Symbol::Interval")) {
            $interval = $obj;

            DEBUX(5, "%s is an interval", $interval);
        }
        elsif (UNIVERSAL::isa($obj, "FSA::Symbol")) {
            $interval = new FSA::Symbol::Interval($obj);

            DEBUX(5, "%s transformed to %s", $obj, $interval);
        }
        else {
            FATAL("%s isn't a FSA::Symbol::Interval nor a FSA::Symbol", $obj);
        }
...
sub add($@) {
    ...
    my @interval = FSA::Symbol::Interval::union($this->_interval(), @add);
...
sub remove($@) {
    ...
    my @interval = map(FSA::Symbol::Interval::subtraction($_, @remove), $this->_interval());
...
sub union(@) {
    ...
    my @interval_union = FSA::Symbol::Interval::union(map($_->_interval(), @_));

    my $union = new FSA::Symbol::Set(@interval_union);
...
sub _intersection2($$) {
    ...
    foreach my $interval1 (@intervals1) {
        foreach my $interval2 (@intervals2) {
            my $intersect_interval = FSA::Symbol::Interval::intersection($interval1, $interval2);

            push(@intersect_intervals, $intersect_interval);
        }
    }

    @intersect_intervals = FSA::Symbol::Interval::union(@intersect_intervals);

    return new FSA::Symbol::Set(@intersect_intervals);
}
...
sub intersection(@) {
    ...
    my $intersect = shift || return new FSA::Symbol::Set;
...
sub subtraction($@) {
    ...
    my @right_intervals_union = FSA::Symbol::Interval::union(map($_->_interval(), @_));
    ...
    foreach my $left_interval (@left_intervals) {
        ...
        my @left_sub = FSA::Symbol::Interval::subtraction($left_interval, @right_intervals_union);
    ...
    @sub_intervals = FSA::Symbol::Interval::union(@sub_intervals);

    my $subtract = new FSA::Symbol::Set(@sub_intervals);
...
sub complement($) {
    ...
    my @co = FSA::Symbol::Interval::complement($this->_interval());

    my $co = new FSA::Symbol::Set(@co);
