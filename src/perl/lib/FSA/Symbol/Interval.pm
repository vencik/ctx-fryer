...
sub copy($) {
    ...
    return new FSA::Symbol::Interval($this->lo(), $this->hi());
}
...
sub lo($@) {
    ...
    if (@_) {
        ...
        UNIVERSAL::isa($lo, "FSA::Symbol") or FATAL("$lo is not FSA::Symbol");

        my $hi = $this->{_hi};

        $hi or $this->{_hi} = $hi = $lo;

        FSA::Symbol::commonAncestor($lo, $hi) or FATAL("$lo and $hi are not compatible");
...
sub hi($@) {
    ...
    if (@_) {
        ...
        UNIVERSAL::isa($hi, "FSA::Symbol") or FATAL("$hi is not FSA::Symbol");
...
sub _intersection2($$) {
    ...
    if ($interval1->hi() < $interval2->lo()) {
        return new FSA::Symbol::Interval();
    }

    # Overlay
    return new FSA::Symbol::Interval($interval2->lo(), $interval1->hi());
}
...
sub intersection(@) {
    ...
    @_ || return new FSA::Symbol::Interval();
...
sub _union2($$) {
    ...
    return new FSA::Symbol::Interval($lo, $hi);
}
...
sub _complement1($) {
    ...
    my $base = FSA::Symbol::commonAncestor($lo, $hi);
    ...
    defined $sym && push(@complement, new FSA::Symbol::Interval($first, $sym));

    $sym = $this->hi()->succ();

    defined $sym && push(@complement, new FSA::Symbol::Interval($sym, $last));
