...
use CTXFryer::FSA::Symbol::Set;

use CTXFryer::FSA::Branch;
use CTXFryer::FSA::State;
...
sub new($$@) {
    ...
    UNIVERSAL::isa($arg, "CTXFryer::FSA::Regex")
    and return CTXFryer::FSA::Regex::fsa($arg);
    ...
    my $root = shift || new CTXFryer::FSA::State($arg . " root");
...
sub accepts($$@) {
    ...
    $symbol_class ||= "CTXFryer::FSA::Symbol::ASCII";
...
sub _unionBranch($$$$$$) {
    ...
    unless ($target) {
        $target = new CTXFryer::FSA::State($target_desc);
...
sub _union2($$) {
    ...
    my $this = new CTXFryer::FSA("U(" . $fsa1->desc() . ", " . $fsa2->desc() . ")");
    ...
    while (@state_queue) {
        ...
        if (defined $state1) {
            ...
            @branches1 = map([ CTXFryer::FSA::Symbol::Set::copy($_->symbol()), $_->target() ], $state1->branch());
        }
        ...
        if (defined $state2) {
            ...
            @branches2 = map([ CTXFryer::FSA::Symbol::Set::copy($_->symbol()), $_->target() ], $state2->branch());
        }
        ...
        while (@branches1) {
            ...
            foreach my $branch2 (@branches2) {
                my $intersect = CTXFryer::FSA::Symbol::Set::intersection($symbol1, $branch2->[0]);
                ...
                unless ($intersect->isEmpty()) {
                    ...
                    $symbol1 = CTXFryer::FSA::Symbol::Set::subtraction($symbol1, $intersect);

                    $branch2->[0] = CTXFryer::FSA::Symbol::Set::subtraction($branch2->[0], $intersect);
                }
...
sub _addRootBranchResidual($$) {
    ...
    my $symbol_exclude = CTXFryer::FSA::Symbol::Set::union(map($_->symbol(), $state->branch()));
    ...
    foreach my $root_branch ($fsa->root()->branch()) {
        my $symbol = CTXFryer::FSA::Symbol::Set::subtraction($root_branch->symbol(), $symbol_exclude);
        ...
        if (@added_branches) {
            ...
            foreach my $added_branch (@added_branches) {
                ...
                foreach my $current_branch (@current_branches) {
                    ...
                    my $intersect_symbol = CTXFryer::FSA::Symbol::Set::intersection($added_symbol, $current_symbol);
                    ...
                    unless ($intersect_symbol->isEmpty()) {
                        ...
                        my $intersect_state = new CTXFryer::FSA::State($current_target->desc() . "." . $added_target->desc());
                        ...
                        my $intersect_branch = new CTXFryer::FSA::Branch;
                        ...
                        $current_symbol = CTXFryer::FSA::Symbol::Set::subtraction($current_symbol, $intersect_symbol);
                        $added_symbol   = CTXFryer::FSA::Symbol::Set::subtraction($added_symbol,   $intersect_symbol);
                    }
...
sub iteration($$@) {
    ...
    for (; $i < $lo; ++$i) {
        $iter = CTXFryer::FSA::concat($iter, $fsa);
    }
...
sub copy($) {
    ...
    $this->{_desc} = $fsa->desc();
    $this->{_root} = CTXFryer::FSA::State::copy($fsa_root);
    ...
    while (@states) {
        ...
        foreach my $orig_branch (@branches) {
            my $copy_branch = CTXFryer::FSA::Branch::copy($orig_branch);
            ...
            unless ($copy_target) {
                $copy_target = CTXFryer::FSA::State::copy($orig_target);
...
sub minimise($) {
    ...
    foreach my $state (@states) {
        ...
        foreach my $symbol (map($_->symbol()->copy(), $state->branch())) {
            ...
            foreach my $enlisted (@symbol_list) {
                my $intersection = CTXFryer::FSA::Symbol::Set::intersection($enlisted, $symbol);

                unless ($intersection->isEmpty()) {
                    push(@symbols, $intersection);

                    $enlisted = CTXFryer::FSA::Symbol::Set::subtraction($enlisted, $intersection);
                    $symbol   = CTXFryer::FSA::Symbol::Set::subtraction($symbol,   $intersection);
        ...
        for (; $i < @state_tab; ++$i) {
            ...
            CTXFryer::FSA::State::cmpByAccept($representant, $state) || last;
        }
    ...
    foreach my $state (@states) {
        ...
        foreach my $symbol (@symbol_list) {
            ...
            foreach my $branch ($state->branch()) {
                my $intersect = CTXFryer::FSA::Symbol::Set::intersection($symbol, $branch->symbol());
    ...
    for (my $i = 0; $i < @state_tab; ++$i) {
        ...
        foreach my $branch ($representant->branch()) {
            ...
            if (exists $min_branches{$min_branch_target}) {
                ...
                $min_branch->symbol(CTXFryer::FSA::Symbol::Set::union($min_branch_symbol, $symbol));
            }
            else {
                my $min_branch_symbol = $symbol->copy();

                my $min_branch = new CTXFryer::FSA::Branch;
    ...
    my $min_fsa = new CTXFryer::FSA($this->desc() . " minimal", $min_root);
