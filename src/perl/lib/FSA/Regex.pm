...
sub _ASCII_complement($) {
    ...
    return join("", grep(index($chars, $_) == -1, @FSA::Symbol::ASCII::all));
}


BEGIN {
    # Create ASCII symbol map
    %ASCII_map = map(($_ => new FSA::Symbol::ASCII($_)), @FSA::Symbol::ASCII::all);
...
sub _parse($@) {
    ...
    for (; $pos < $len; ++$pos) {
        ...
        # Sub-regex
            ...
            my $sub_regex = new FSA::Regex(substr($def, $pos + 1),
                                           "-base"     => $this->{_base},
                                           "-map"      => $this->{_map},
                                           "-esc"      => $this->{_esc},
                                           "-level"    => $this->{_level} + 1,
        ...
        if ($set) {
            ...
            if ($set eq "done") {
                ...
                my $sym_spec = new FSA::Symbol::Set(@set);

                if ($set_neg) {
                    if (@set) {
                        $sym_spec = FSA::Symbol::Set::complement($sym_spec);
                    }
                    else {
                        my $lo = $this->base()->first();
                        my $hi = $this->base()->last();

                        my $interval = new FSA::Symbol::Interval($lo, $hi);

                        $sym_spec = new FSA::Symbol::Set($interval);
                    }
                }
            ...
            # Set symbol defined
                ...
                if ($set_range) {
                    ...
                    my $interval = new FSA::Symbol::Interval($lo, $sym);
        ...
        # Symbol defined
            ...
            # Create symbol specification
            my $sym_spec = new FSA::Symbol::Set($sym);
...
sub fsa($) {
    ...
    for (my $a = 0; $a < @$dom_tree; ++$a) {
        my $alt = new FSA("\"$def\" alternative $a");
        ...
        for (my $i = 0; $i < @$iters; ++$i) {
            ...
            if (UNIVERSAL::isa($obj, "FSA::Symbol::Set")) {
                $sub = new FSA("\"$def\" alternative $a symbol set iteration $i");

                my $branch = new FSA::Branch;
                my $target = new FSA::State("\"$def\" alternative $a symbol set iteration $i");
            ...
            # Sub-regex iteration
            elsif (UNIVERSAL::isa($obj, "FSA::Regex")) {
                $sub = $obj->fsa();
            }

            # Internal error
            else {
                FATAL("Iteration object %s isn't a FSA::Symbol::Set nor FSA::Regex", $obj);
            }

            DEBUX(2, "\"$def\" alternative $a iteration $i subject: %s", $sub);

            my $sub_iter = FSA::iteration($sub, $lo, $hi);

            DEBUX(2, "\"$def\" alternative $a iteration $i (%d..%d): %s", $lo, $hi, $sub_iter);

            $alt = FSA::concat($alt, $sub_iter);
        }
...
sub str($) {
    ...
    if ($debug && $debug > 1) {
        ...
        foreach my $char (sort keys %{$this->{_map}}) {
            ...
            my $char_str = FSA::Symbol::ASCII::printable($char);
        ...
        foreach my $char (sort keys %{$this->{_esc}}) {
            ...
            my $printable_str = FSA::Symbol::ASCII::printable($str);
