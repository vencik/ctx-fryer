...
use LRParser::Action::Shift;
use LRParser::Action::Reduce;
use LRParser::Action::Accept;

use LRParser::State;
use LRParser::Output;

use LRParser::Transition;
use LRParser::Reduction;

use Grammar::Terminal::EOF;
...
sub _createLR0itemSets($) {
    ...
    my $item_sets = new List;
    my $sym_tab   = new Table;
    my $trans_tab = new Table(2);

    my $item_set_0 = new Grammar::ItemSet(
         new Grammar::Item($root_rule, 0));
    ...
    for (my $i = 0; $i < $item_sets->size(); ++$i) {
        ...
        while (my ($sym_id, $kernel) = each %kernels) {
            $item_set  = new Grammar::ItemSet(@$kernel);
...
sub new($$) {
    ...
    for (my $i = 0; $i < $item_sets->size(); ++$i) {
        push(@states, new LRParser::State($i, $item_sets->at($i)));
    }

    # Copy the transitions to action and goto tables
    my $action_tab = new Table(2);
    my $goto_tab   = new Table(2);

    my $iter = new Table::Iterator($trans_tab);

    while (my ($i, $sym_id, $target) = $iter->each()) {
        my $symbol = $sym_tab->at($sym_id);

        # Terminal transitions form shift actions
        if (UNIVERSAL::isa($symbol, "Grammar::Terminal")) {
            $action_tab->at($i, $sym_id,
                new List(
                    new LRParser::Action::Shift($target)));
        }

        # Non-terminal transitions form the goto table
        elsif (UNIVERSAL::isa($symbol, "Grammar::NonTerminal")) {
    ...
    grep {
        UNIVERSAL::isa($sym_tab->at(@$_), "Grammar::Terminal")
    } $sym_tab->keys();

    push(@terminal_ids, $eof->id());

    my $accept_item = new Grammar::Item($root_rule, $root_rule->length());

    my $shift_reduce_conflicts  = new List;
    my $reduce_reduce_conflicts = new List;
    ...
    for (my $i = 0; $i < $item_sets->size(); ++$i) {
        ...
        if (1 < @reduce_rule_no) {
            ...
            $reduce_reduce_conflicts->push(map(new List($i, $_), @terminal_ids));
        }
        ...
        foreach my $term_id (@terminal_ids) {
            ...
            if ($action_list) {
                ...
                UNIVERSAL::isa($action_list->at(0), "LRParser::Action::Shift")
                ...
                $shift_reduce_conflicts->push(new List($i, $term_id));
            }

            # Only action
            else {
                $action_list = new List;

                $action_tab->at($i, $term_id, $action_list);
            }

            my @reduce = map(new LRParser::Action::Reduce($_), @reduce_rule_no);
    ...
    $this->{_output}     = new LRParser::Output;
...
sub _func($$$$) {
    ...
    # Shift
    if ($action->isa("LRParser::Action::Shift")) {
        $input->shift();
    ...
    # Reduce
    elsif ($action->isa("LRParser::Action::Reduce")) {
    ...
    # Accept
    elsif ($action->isa("LRParser::Action::Accept")) {
        # Sanity checks
        $next_term == $eof
...
sub _terminalTransitions($@) {
    ...
    my $term_trans_tab = $this->_transitionTable()->cut($state_no, sub {
        ...
        UNIVERSAL::isa($symbol, "Grammar::Terminal")
    });
...
sub _directRead($) {
    ...
    my $goto_tab_iter = new Table::Iterator($goto_tab);
    ...
    while (my ($p, $A, $r) = $goto_tab_iter->each()) {
        ...
        push(@DR, new LRParser::Transition($p, $A),
                  new Math::Set(@terminals));
    }

    return new Math::Function::Unary(@DR);
}
...
sub _reads($) {
    ...
    while (my ($p, $A, $r) = $goto_tab_iter->each()) {
        ...
        my $r_nt_trans_iter = new Table::Iterator($r_nonterminal_trans);

        while (my ($C, $q) = $r_nt_trans_iter->each()) {
            my $C_nt = $sym_tab->at($C);

            # Sanity check
            UNIVERSAL::isa($C_nt, "Grammar::NonTerminal")
            or FATAL("INTERNAL ERROR: %s isn't a non-terminal", $C_nt);

            if ($C_nt->isNullable()) {
                my $pA = new LRParser::Transition($p, $A);
                my $rC = new LRParser::Transition($r, $C);

                push(@reads, new Math::Couple($pA, $rC));
            }
        }
    }

    return new Math::Relation::Binary(@reads);
}
...
sub _traverseTransBackwards($$@) {
    ...
    while (@_) {
        ...
        $states = new Math::Set(@prev_states);
    }
...
sub _includes($) {
    ...
    foreach my $pA ($goto_tab->keys()) {
        ...
        # Sanity check
        UNIVERSAL::isa($A, "Grammar::NonTerminal")
        ...
        foreach my $rule_no ($A->RHSrules()) {
            ...
            while (@right) {
                ...
                # Sanity check
                UNIVERSAL::isa($symbol, "Grammar::NonTerminal")
            ...
            foreach my $q ($states->items()) {
                if (defined $trans_tab->at($q, $B_id)) {
                    push(@includes,
                        new Math::Couple(
                            new LRParser::Transition($p, $A_id),
                            new LRParser::Transition($q, $B_id)));
                }
            }
        }
    }

    return new Math::Relation::Binary(@includes);
}
...
sub _lookback($) {
    ...
    my $reduce_tab = $action_tab->cut(undef, undef, sub {
        ...
        foreach my $action ($actions->items()) {
            $reduce = UNIVERSAL::isa($action, "LRParser::Action::Reduce");
    ...
    my $reduce_tab_iter = new Table::Iterator($reduce_tab);

    while (my ($q, $t_id, $actions) = $reduce_tab_iter->each()) {
        foreach my $action ($actions->items()) {
            UNIVERSAL::isa($action, "LRParser::Action::Reduce") || next;

            my $reduction = new LRParser::Reduction($q, $action->rule());
    ...
    my $reductions = new Math::Set(@reductions);
    ...
    foreach my $reduction ($reductions->items()) {
        ...
        foreach my $p ($states->items()) {
            if (defined $trans_tab->at($p, $A_id)) {
                ...
                push(@lookback,
                    new Math::Couple(
                        $reduction, new LRParser::Transition($p, $A_id)));
            }
        }
    }

    return new Math::Relation::Binary(@lookback);
}
    ...
    for (my $q = 0; $q < @$states; ++$q) {
        ...
        for (my $i_idx = 0; $i_idx < @terminals - 1; ++$i_idx) {
            ...
            for (my $j_idx = $i_idx + 1; $j_idx < @terminals; ++$j_idx) {
                ...
                my $LA_intersect = Math::Set::intersection($LA_qi, $LA_qj);
...
sub _invalidateNonLALR1actions($) {
    ...
    my $action_tab_iter = new Table::Iterator($action_tab);
    ...
    while (my ($state, $terminal_id, $actions) = $action_tab_iter->each()) {
        ...
        if (UNIVERSAL::isa($actions[0], "LRParser::Action::Accept")) {
            1 == @actions
        ...
        if (UNIVERSAL::isa($actions[0], "LRParser::Action::Shift")) {
            $shift = shift(@actions);
        }
        ...
        foreach my $reduce (@actions) {
            UNIVERSAL::isa($reduce, "LRParser::Action::Reduce")
