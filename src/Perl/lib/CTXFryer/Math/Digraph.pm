...
sub new($$$) {
    ...
    UNIVERSAL::isa($vertices, "CTXFryer::Math::Set")
    or FATAL("INTERNAL ERROR: %s isn't a set", $vertices);

    UNIVERSAL::isa($edges, "CTXFryer::Math::Relation::Binary")
    or FATAL("INTERNAL ERROR: %s isn't a binary relation", $edges);

    # Sanity check
    foreach my $couple ($edges->items()) {
        UNIVERSAL::isa($couple, "CTXFryer::Math::Couple")
...
sub _determineSCC_traverse($$$$$$$$$@) {
    ...
    my $vertex_key   = CTXFryer::Math::Set::itemKey($vertex);
    ...
    if ($$vertex_depth == $initial_vertex_depth) {
        ...
        do {
            ...
            $popped_vertex_key = CTXFryer::Math::Set::itemKey($popped_vertex);
        ...
        push(@$sccs, new CTXFryer::Math::Set(@scc));
    }
...
sub determineSCC($@) {
    ...
    my %depths   = map((CTXFryer::Math::Set::itemKey($_) => 0), @vertices);
