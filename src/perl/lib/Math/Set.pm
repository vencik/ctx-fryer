...
sub isSubset($$) {
    ...
    UNIVERSAL::isa($set, "Math::Set")
    or FATAL("INTERNAL ERROR: %s isn't a Math::Set", $set);
...
sub subtractionAssign($@) {
    ...
    foreach my $set (@_) {
        ...
        my $iter = new Table::Iterator($set->{_impl});
...
sub subtraction($@) {
    ...
    my $iter = new Table::Iterator($left->{_impl});
