...
sub _addItem($@) {
    ...
    while (@_) {
        ...
        push(@rel_items, new Math::Multiplet($args->item(), $val));
    }
...
sub definitionRange($) {
    ...
    foreach my $multiplet ($this->items()) {
        ...
        my $args = new Math::Multiplet(@args);

        push(@def_range, $args);
    }

    return new Math::Set(@def_range);
}
...
sub project($$) {
    ...
    # Sanity checks
    UNIVERSAL::isa($args, "Math::Multiplet")
