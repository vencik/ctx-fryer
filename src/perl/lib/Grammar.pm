...
use ProjectDef::Grammar::TerminalSymbols;
use ProjectDef::Grammar::Rules;
use ProjectDef::Grammar::Attributes;

use Grammar::Terminal;
use Grammar::NonTerminal;
use Grammar::Rule;
use Grammar::Symbol::Attribute qw(:all);

use Grammar::Symbol::Attribute::Function;
use Grammar::Symbol::Attribute::Reference;
use Grammar::Symbol::Attribute::Dependency;
use Grammar::Symbol::Attribute::Destructor;
...
sub new($$$$$) {
    ...
    # Sanity checks
    UNIVERSAL::isa($terminals_def, "ProjectDef::Grammar::TerminalSymbols")
    or FATAL("INTERNAL ERROR: Terminal symbols argument is invalid");

    UNIVERSAL::isa($rules_def, "ProjectDef::Grammar::Rules")
    or FATAL("INTERNAL ERROR: Rules argument is invalid");

    UNIVERSAL::isa($attrs_def, "ProjectDef::Grammar::Attributes")
    or FATAL("INTERNAL ERROR: Attributes argument is invalid");

    # Create non-terminals table
    my $non_terminals = new Table;

    foreach my $ident (@$non_terminals_def) {
        $non_terminals->at($ident, new Grammar::NonTerminal($ident));
    }

    # Create terminals table
    my %terminals = $terminals_def->map();
    my $terminals = new Table;

    while (my ($ident, $regex) = each %terminals) {
        my $terminal = new Grammar::Terminal($ident, $regex);
    ...
    foreach my $rule (@rules) {
        ...
        $rules->push(new Grammar::Rule($left_non_terminal => @right_symbols));
    }
    ...
    my $token_getter = new Grammar::Symbol::Attribute::Function("*", 0, "builtin::get_token");

    foreach my $terminal ($terminals->values()) {
        my $token = new Grammar::Symbol::Attribute(ATTR_TYPE_AGGREGATED, "token");
    ...
    if ($attrs_def) {
        ...
        foreach my $attr_def ($attrs_def->explicit()) {
            # Sanity check
            UNIVERSAL::isa($attr_def, "ProjectDef::Grammar::Attribute::Explicit")
            ...
            if ($symbol) {
                my $attr = new Grammar::Symbol::Attribute($attr_type, $attr_ident);

                # Add destructor (if exists)
                my $destroy = $attrs_def->destructor($sym_ident . '::' . $attr_ident);

                $destroy && $attr->destructor(new Grammar::Symbol::Attribute::Destructor($destroy));
        ...
        foreach my $attr_def ($attrs_def->implicit()) {
            # Sanity check
            UNIVERSAL::isa($attr_def, "ProjectDef::Grammar::Attribute::Implicit")
            ...
            if ($symbol) {
                ...
                if ($attr) {
                    ...
                    $attr = new Grammar::Symbol::Attribute($attr_type, $attr_ident);
    ...
    foreach my $attr_and_def (@attr_and_def) {
        ...
        foreach my $arg_def ($attr_def->arguments()) {
            ...
            DEP_DEF: {  # implement by pragmatic do while (0) loop
                ...
                $dep = new Grammar::Symbol::Attribute::Dependency($dep_attr->identifier(), $sym_no);

            }  # end of DEP_DEF block

            $dep ||= $Grammar::Symbol::Attribute::Dependency::undef;
        ...
        if ($eval_def eq "") {
            ...
            $eval = new Grammar::Symbol::Attribute::Reference($rule_id, $attr_sym_no, @deps);
        }

        # Function
        else {
            $eval = new Grammar::Symbol::Attribute::Function($rule_id, $attr_sym_no, $eval_def, @deps);
        }
...
sub reduce($) {
    ...
    foreach my $rule ($this->rule()) {
        my $right_nt_count = grep {
            UNIVERSAL::isa($_, "Grammar::NonTerminal")
        } $rule->right();
    ...
    foreach my $nt (values %r2_non_terminals) {
        my $nt_copy = new Grammar::NonTerminal($nt->ident());
    ...
    my @terminals = grep {
        UNIVERSAL::isa($_, "Grammar::Terminal")
    }
    map {
        $_->right()
    }
    ...
    foreach my $t (@terminals) {
        my $t_copy = new Grammar::Terminal($t->ident(), $t->regex());
    ...
    foreach my $rule (sort { $a->number() <=> $b->number() } values %r2_rules) {
        ...
        my $rule_copy = new Grammar::Rule($left => @right);
...
sub augmented($) {
    ...
    # Add new root
    my $new_root = new Grammar::NonTerminal("_" . $root->ident());

    # Copy non-terminals
    my $new_non_terminals = new Table;
    ...
    # Copy rules
    my $new_rules = new List(new Grammar::Rule($new_root => $symbol_map{$root->id()}));
...
sub _markNullableNonTerminals($) {
    ...
    my @rules = grep {
        ...
        while ($nt_only && @right) {
            ...
            $nt_only &&= UNIVERSAL::isa($symbol, "Grammar::NonTerminal");
        }
...
sub xmlChildren($) {
    ...
    return (
        xmlNewElement("non-terminals", {}, new List($non_terminals->values())),
        xmlNewElement("terminals",     {}, new List($terminals->values())),
