package Math::Function::Unary;

use strict;
use warnings;

use base qw(Math::Function);
...
sub new($@) {
    ...
    while (@_) {
        ...
        push(@def, (new Math::Multiplet($arg) => $val));
    }
...
sub definitionRange($) {
    ...
    return new Math::Set(map($_->item(), @def_range_1plets));
}
...
sub project($$) {
    ...
    return $this->SUPER::project(new Math::Multiplet($arg));
}
...
sub _closure_kernel($$) {
    ...
    my $arg_key = Math::Set::itemKey($arg);
    ...
    if (UNIVERSAL::isa($kernel, "Math::Set")) {
        $closure->{$arg_key} = $kernel->copy();
    }
    else {
        $closure->{$arg_key} = new Math::Set($kernel);
    }
}
...
sub _closure_growth($$$$) {
    ...
    my $arg_key     = Math::Set::itemKey($arg);
    my $rel_arg_key = Math::Set::itemKey($rel_arg);
...
sub closure($@) {
    ...
    # Sanity checks
    UNIVERSAL::isa($rel, "Math::Relation")
    ...
    my $digraph = new Math::Digraph($def_range, $rel);
