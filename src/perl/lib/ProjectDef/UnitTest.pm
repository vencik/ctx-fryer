...
use ProjectDef::UnitTest::TestCase;
...
sub parse($$$) {
    ...
    if (UNIT_TEST_SCOPE == $scope && /^\s*TestCase\s*$/i) {
        ...
        push(@{$this->{test_cases}},
            new ProjectDef::UnitTest::TestCase);
    }
    ...
    # Word
        ...
        $this->{test_cases}->[-1]->word(ProjectDef::unescape($1));
    }
    ...
    # Multi-line word definition
        ...
        $this->{test_cases}->[-1]->word(ProjectDef::unescape($_));
    }

    # Derivation
    elsif (TEST_CASE_SCOPE == $scope && /^\s*derivation\s*=\s*(([0-9]+\s?){1,})\s*$/i) {
        $this->{test_cases}[-1]->derivation(ProjectDef::str2list(" ", $1));
    }
    ...
    # Derivation definition
        ...
        $this->{test_cases}->[-1]->derivation(ProjectDef::str2list(", ", $line));
    }

    # Accept flag
    elsif (TEST_CASE_SCOPE == $scope && /^\s*accept\s*=\s*(.*)\s*$/i) {
        $this->{test_cases}->[-1]->accept(ProjectDef::str2bool($1));
    }
