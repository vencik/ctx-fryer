package CTXFryer::ProjectDef::Grammar::Attributes;
...
use CTXFryer::ProjectDef::Grammar::Attribute::Explicit;
use CTXFryer::ProjectDef::Grammar::Attribute::Implicit;
...
sub parse($$$) {
    ...
    if ($line =~ /^\s*(\S*)\s*=\s*(\S*)\s*\((.*)\)\s*$/) {
        my $name = $1;
        my $func = $2;
        my @args = CTXFryer::ProjectDef::str2list(", ", $3);

        my $attr = new CTXFryer::ProjectDef::Grammar::Attribute::Explicit(
                       name      => $name,
                       function  => $func,
                       arguments => [ @args ],
                       );
    ...
    # Implicit attribute definition
        ...
        my $name    = $4;
        my $func    = $5;
        my @args    = CTXFryer::ProjectDef::str2list(", ", $6);

        my $attr = new CTXFryer::ProjectDef::Grammar::Attribute::Implicit(
                       name      => $name,
                       "rule-id" => $rule_id,
                       "rule-no" => $rule_no,
                       function  => $func,
