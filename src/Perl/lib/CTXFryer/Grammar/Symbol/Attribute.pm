package CTXFryer::Grammar::Symbol::Attribute;
...
use CTXFryer::Grammar::Symbol::Attribute::Evaluator;
...
@ISA = qw(CTXFryer::Identified CTXFryer::Serialised Exporter);
...
sub new($$$@) {
    ...
    foreach my $eval (@evals) {
        UNIVERSAL::isa($eval, "CTXFryer::Grammar::Attribute::Evaluator")
        or FATAL("INTERNAL ERROR: Unexpected evaluator type: %s", ref($eval));
    }

    if (defined $destr) {
        UNIVERSAL::isa($destr, "CTXFryer::Grammar::Attribute::Destructor")
    ...
    $this->{_type}  = $type;
    $this->{_ident} = $ident;
    $this->{_eval}  = new CTXFryer::List(@_);
    $this->{_destr} = $destr;
...
sub copy($$) {
    ...
    $this->{_eval} = new CTXFryer::List(@eval);
